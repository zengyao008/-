...
from ai_module import generate_risk_report, build_analysis_prompts
...
if page == "首页介绍":
    ...
    col2.metric("口径统一", "100%对齐", "标准化统计规则")
    ...
    if st.session_state["analysis_result"] is None:
        ...
        with tab3:
            ...
            with col_right:
                ...
                loss_df = res["loss_rates"].copy()
                # 仅做百分比格式化，保留2位小数
                loss_styled = loss_df.style.format("{:.2%}")
                st.dataframe(loss_styled, width='stretch')

            st.subheader("月度拨备汇总")
            prov_df = res["monthly_provision"].copy()
            prov_df = prov_df.reset_index(names="Vintage")  #索引转普通列，命名Vintage
            # 仅做单位与格式统一：金额转万元、拨备率百分比
            prov_styled = prov_df.style.format({
                "月末总资产余额": lambda x: f"{x/10000:,.2f} 万元",
                "当月应计提拨备": lambda x: f"{x/10000:,.2f} 万元",
                "应计提拨备率": "{:.2%}"
            })
            st.dataframe(prov_styled, width='stretch', hide_index=True)
        ...
        with tab4:
            ...
            detail_df = first_ovd.copy().reset_index(drop=True)
            # 新增从1开始的连续序号，替代原生杂乱索引
            detail_df.insert(0, "序号", range(1, len(detail_df) + 1))

            # 可选：保留核心列，页面更清爽；不需要可以注释掉
            core_cols = [
                "序号", "合同号", "客户名称", "经销商名称", "大区",
                "首次逾期期号", "逾期天数（DPD）", "未偿还本金", "Vintage"
            ]
            show_cols = [col for col in core_cols if col in detail_df.columns]
            detail_df = detail_df[show_cols]

            # 仅做数值格式化：金额转万元、天数/期号取整
            detail_styled = detail_df.style.format({
                "未偿还本金": lambda x: f"{x/10000:,.2f} 万元",
                "逾期天数（DPD）": "{:.0f}",
                "期号": "{:.0f}",
                "序号": "{:d}"
            })
            ...
            st.dataframe(
                detail_styled,
                width='stretch',
                hide_index=True,  # 隐藏原生0开始的数字索引
                height=420        # 固定高度，数据多了自动滚动
            )
...
# ========== 4. 风险预警中心 ==========
    ...
    if st.session_state["analysis_result"] is None:
        ...
        with tab1:
            ...
            detail_continuous = continuous_df.copy().reset_index(drop=True)
            # 新增从1开始的连续序号
            detail_continuous.insert(0, "序号", range(1, len(detail_continuous) + 1))

            # 仅做数值格式化：金额转万元、期数取整
            detail_continuous_styled = detail_continuous.style.format({
                "未偿还本金": lambda x: f"{x/10000:,.2f} 万元",
                "逾期天数（DPD）": "{:.0f}",
                "连续升档期数": "{:.0f}",
                "序号": "{:d}"
        ...
        with tab2:
            ...
            st.markdown("---")
            st.subheader("大额逾期明细")
            detail_top = top_overdue.copy().reset_index(drop=True)
            # 新增从1开始的连续序号
            detail_top.insert(0, "序号", range(1, len(detail_top) + 1))

            # 仅做数值格式化：金额转万元、天数取整
            detail_top_styled = detail_top.style.format({
                "未偿还本金": lambda x: f"{x/10000:,.2f} 万元",
                "逾期天数（DPD）": "{:.0f}",
                "序号": "{:d}"
            })
...
# ========== 5. AI智能解读 ==========
    ...
    if st.session_state["analysis_result"] is None:
        ...
        res = st.session_state["analysis_result"]  # 新增这一行

        # 操作按钮区
        col_btn1, col_btn2 = st.columns([1, 1])
        with col_btn1:
            if st.button("🤖 生成AI分析报告", type="primary", use_container_width=True):
                with st.spinner("正在生成专业分析报告..."):
                    st.session_state["ai_report"] = generate_risk_report(res)

        with col_btn2:
            if st.button("📝 查看提示词原文", use_container_width=True):
                sys_prompt, user_prompt = build_analysis_prompts(res)
                st.session_state["prompt_system"] = sys_prompt
                st.session_state["prompt_user"] = user_prompt

        # 提示词展示区（折叠收起）
        if "prompt_system" in st.session_state and "prompt_user" in st.session_state:
            with st.expander("🔧 完整提示词原文（调试/降级使用）", expanded=False):
                st.markdown("**系统提示词（system_prompt）**")
                st.code(st.session_state["prompt_system"], language=None)
                st.markdown("---")
                st.markdown("**用户数据提示词（user_prompt）**")
                st.code(st.session_state["prompt_user"], language=None)
                st.caption("💡 API调用失败时，可复制以上内容手动生成报告")
